...
class ZhonghuayingcaiwangPipeline(object):
    def process_item(self, item, spider):
        return item


# 此网站一个反爬策略为提供干扰信息，此类用来过滤掉无关信息
class Filter(object):
    def process_item(self, item, spider):
        if spider.name == 'php':
            if 'php'in item['jobName'] or 'PHP'in item['jobName'] or '后端'in item['jobName'] or '网站'in item['jobName'] or 'web' in item['jobName'] or '网页' in item['jobName']:
                return item
            else:
                raise DropItem(item)
